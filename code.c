void code()
{
	...
	moveArm(50, 50, 30, 0.2, LIFT, RELDEG); // Lowers the claw so the new stacks rest on the old ones
	...
	if (!startedClose)
	{
		...
		startTask(liftUp1000);
		moveSimpleAcc(30, 30, 86, 0.15, RELDEG);
	...
	startTask(clawWater);
	moveArm(-50, -50, 1000, 0.01, LIFT, TIME);
